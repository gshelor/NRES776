...
## also downloading Daymet data for temp and precip info

##### Loading Packages #####
library(pacman)
p_load(snotelr, here, tidyverse, randomForest, caret, ranger, rstan, leaflet, shinydashboard, plotly, DT, daymetr, ncdf4, parallel, utils, svMisc)
...
SnotelClean <- Snoteldata_AllSites |>
  ...
  mutate(date = paste0(Year, "-", Month, "-", Day), .before = 15)
...
## adding new columns to assist with grabbing daymet data
SnotelClean <- SnotelClean |>
  ...
  mutate(study_year = Year - min(Year),
         daymet_maxtemp = -999,
         daymet_mintemp = -999,
         daymet_precip = -999) |>
  arrange(date, site_id)

## writing csv in case script crashes
...
##### Grabbing Daymet Data #####
## extracting unique SNOTEL site ids
SnotelSiteID <- sort(unique(SnotelClean$site_id))

## setting timer to see how long this takes
daymet_starttime <- Sys.time()
...
## end time
daymet_endtime <- Sys.time()
daymet_downloadtime <- daymet_endtime - daymet_starttime
daymet_downloadtime

## adding mean temp column
SnotelDaymet <- SnotelDaymet |>
  mutate(daymet_meantemp = (daymet_maxtemp + daymet_mintemp) / 2)

poopypants <- SnotelDaymet |>
  filter(daymet_maxtemp == -999 | daymet_mintemp == -999 | daymet_meantemp == -999)

poopypants_NA <- SnotelDaymet |>
  filter(is.na(daymet_maxtemp))
poopypants_NAmin <- SnotelDaymet |>
  filter(is.na(daymet_mintemp))
poopypants_NAmean <- SnotelDaymet |>
  filter(is.na(daymet_meantemp))

SnotelDaymetClean <- SnotelDaymet |>
  drop_na(daymet_maxtemp) |>
  drop_na(daymet_mintemp) |>
  drop_na(daymet_meantemp)


## writing final csvs
write_csv(SnotelDaymet, here("Project", "Data", "SNOTEL", "SnotelDaymet.csv"))
write_csv(SnotelDaymetClean, here("Project", "Data", "SNOTEL", "SnotelDaymetClean.csv"))

## SnotelDaymetClean.csv now gets loaded into SWEForecasting.R for modeling and analysis
