...
fit1<-stan(file = here("UNR-EcoForecast-main", "lectures", "SS", "StanSSPois.stan"),data=modeldata, chains=3,iter=10000,control=list(adapt_delta=0.99))
...
options(mc.cores = parallel::detectCores())
fit1<-stan(file = here("UNR-EcoForecast-main", "lectures", "SS", "StanSSNorm.stan"),data=modeldata, chains=3,iter=10000)
# launch_shinystan(fit1)
