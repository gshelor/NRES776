...
data_2013_test <- SMwx2013_data |>
  filter(YearDay >= 247)
## storing number of rows in training data to use for Stan models later on
...
# set.seed(802)
# ranger_mod_v7 <- ranger(formula = SoilMoisture ~ Ppt_mm_Tot + Solar_Incoming_Total + Air_TempC_Max + SoilTemp_20cm_Avg + Dewpoint_Avg, data = data_2013_train, num.trees = 10000, mtry = 3, importance = "impurity", quantreg = TRUE)
...
# set.seed(802)
# ranger_pred_v7 <- predict(ranger_mod_v7, data = data_2013_test, type = "quantiles", quantiles = quants)
# set.seed(802)
...
# rangerpredictions_v7 <- data.frame(predictions(ranger_pred_v7))
# plot(data_2013_test$SoilMoisture, col = 'black', ylim = c(0.1,0.35))
...
for (p in 1:length(model1_pars$b0)){
  ...
  for(t in 1:nrow(data_2013_test)){
     SoilMoisture <- rnorm(1, mean = model1_pars$b0[p] + model1_pars$b2[p] * data_2013_test$Air_TempC_Max[t] + model1_pars$b3[p] * data_2013_test$Air_TempC_Min + model1_pars$b4[p] * data_2013_test$Air_TempC_Avg[t] + model1_pars$b5[p] * data_2013_test$Solar_Incoming_Total[t] + model1_pars$b6[p] * data_2013_test$Ppt_mm_Tot[t] + model1_pars$b7[p] * data_2013_test$Relative_Humidity_Max[t] + model1_pars$b8[p] * data_2013_test$Relative_Humidity_Max[t] + model1_pars$b9[p] * data_2013_test$Wind_speed_Max[t] + model1_pars$b10[p] * data_2013_test$WndSpeed_Avg[t] + model1_pars$b11[p] * data_2013_test$WinDir_Avg[t] + model1_pars$b12[p] * data_2013_test$WinDir_Std_Dev[t] + model1_pars$b13[p] * data_2013_test$SoilTemp_5cm_Avg[t] + model1_pars$b14[p] * data_2013_test$SoilTemp_20cm_Avg[t] + model1_pars$b15[p] * data_2013_test$Dewpoint_Avg[t] + model1_pars$b16[p] * data_2013_test$SoilTemp_5cm_Max[t] + model1_pars$b17[p] * data_2013_test$SoilTemp_5cm_Min[t] + model1_pars$b18[p] * data_2013_test$SoilTemp_20cm_Max[t] + model1_pars$b19[p] * data_2013_test$SoilTemp_20cm_Min[t], sd=model1_pars$sigma[p])
...
MeanPred_mod1 <- apply(Pred_out_sm,2,mean)
Upper_mod1 <- apply(Pred_out_sm,2,quantile, prob=.9)
Lower_mod1 <- apply(Pred_out_sm,2,quantile, prob=.1)

## plotting forecasts against data
plot(MeanPred_mod1, type='l', ylim = c(0.05, 0.45),xlim = c(0,30), main = "Applying Final Model on Test Data from 2013", ylab = "Soil Moisture")
lines(Upper_mod1,lty=2)
lines(Lower_mod1,lty=2)
points(data_2013_test$SoilMoisture,col='steelblue')

## RMSE
rmse(data_2013_test$SoilMoisture, MeanPred_mod1)
```
...
## plotting forecasts against data
plot(MeanPred_mod7, type='l', ylim = c(0.05, 0.45),xlim = c(0,30), main = "Applying Final Model on Test Data from 2013", ylab = "Soil Moisture")
